...
namespace dotnetify_react_template
{
  ...
  public class MockLiveDataService : ILiveDataService
  {
    ...
    private System.Diagnostics.Process p = System.Diagnostics.Process.GetCurrentProcess();
    private long ram;
    ...
    {
      // var tt = this._httpContextAccessor.HttpContext.userdata;
      ClaimsPrincipal principal = this._httpContextAccessor.HttpContext.User; // .ClaimsPrincipal; // Identity as ClaimsPrincipal;
      if (null != principal)
      {
        foreach (Claim claim in principal.Claims)
        {
          Console.WriteLine("MockLiveDataService.cs ---------- CLAIM TYPE: " + claim.Type + "; CLAIM VALUE: " + claim.Value + "</br>");
        }
      }

      // var username = _httpContextAccessor.HttpContext.User.Identity.Name;
      Console.WriteLine("MockLiveDataService.cs - _httpContextAccessor.HttpContext.User.Identity.Name ----------- " + (this._httpContextAccessor.HttpContext.User)); // .Value);
      // Console.WriteLine("MockLiveDataService.cs - _httpContextAccessor.HttpContext.User.Identity.NameIdentifier - " + (this._httpContextAccessor.HttpContext.User.Identity.NameIdentifier).Value); // _httpContextAccessor.HttpContext.User.Identity.NameIdentifier); // .Identity.NameIdentifier);
      // Console.WriteLine("MockLiveDataService.cs - _httpContextAccessor.HttpContext.User.Identity.Subject -------- " + (this._httpContextAccessor.HttpContext.User.Identity.Role).Value);
      foreach (Claim claim in this._httpContextAccessor.HttpContext.User.Claims)
      {
        Console.WriteLine("MockLiveDataService.cs - CLAIM TYPE: " + claim.Type + "; CLAIM VALUE: " + claim.Value);
      }
